  ...
  Future<FarmAlerts> _getOfflineAlerts() async {
    ...
    return FarmAlerts(
      healthDue:
          healthDueRows
              .map(
                (row) => HealthDueAlert.fromApi(Map<String, dynamic>.from(row)),
              )
              .toList(),
      calvingDue:
          calvingRows
              .map(
                (row) =>
                    CalvingDueAlert.fromApi(Map<String, dynamic>.from(row)),
              )
              .toList(),
      noMilkToday:
          noMilkRows
              .map(
                (row) =>
                    NoMilkTodayAlert.fromApi(Map<String, dynamic>.from(row)),
              )
              .toList(),
      recentlyTreated:
          recentRows
              .map(
                (row) => RecentlyTreatedAlert.fromApi(
